...
function updatePreviewPlayback() {
  const playable = Boolean(selectedClip() && mediaReady && video.src);
  refs.previewBack.disabled = !playable;
  refs.previewPlay.disabled = !playable;
  refs.previewForward.disabled = !playable;
  const paused = video.paused;
  refs.previewPlay.textContent = paused ? "▶" : "❚❚";
  refs.previewPlay.setAttribute("aria-label", paused ? "Play" : "Pause");
  refs.previewPlay.title = paused ? "Play" : "Pause";
}

function togglePreviewPlayback() {
  if (!selectedClip() || !mediaReady) return;
  if (video.paused) void video.play().catch(() => setStatus("Preview could not start"));
  else video.pause();
}

function nudgePreview(seconds) {
  const clip = selectedClip();
  if (!clip || !mediaReady) return;
  const next = clamp(currentLocalTime() + seconds, 0, Math.max(0, clipDuration(clip) - 0.01));
  video.currentTime = clip.sourceStart + next;
  updateTimeDisplay();
}
...
video.addEventListener("play", updatePreviewPlayback);
video.addEventListener("pause", updatePreviewPlayback);
...
refs.previewPlay.addEventListener("click", togglePreviewPlayback);
refs.previewForward.addEventListener("click", () => nudgePreview(5));
refs.timelineZoom.addEventListener("input", () => {
  timelineEditor.style.setProperty("--timeline-zoom", refs.timelineZoom.value);
});
