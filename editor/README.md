...
- switch between Media and Components tabs, choose a canvas ratio, and export the main timeline plus every branch as one self-contained `.pvo` file.
...
The editor previews a multi-media timeline by switching sources at clip boundaries. Export packages the original source files without transcoding. The PVO player follows the main timeline, waits at an unanswered branching layer, plays only the selected outcome, and ends after that branch.
